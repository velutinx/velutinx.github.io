...
(function() {
  ...
  // --- Cart SVG icons (used on product cards) ---
  ...
  // --- Inject header HTML and initialize components ---
  async function loadHeader() {
    ...
    try {
      ...
      console.error('Failed to load header from /assets/include/top.html:', err);
      // Optional: fallback to inline header (could be added here)
      placeholder.innerHTML = '<div style="padding:1rem; text-align:center;">⚠️ Header failed to load. Please check your network.</div>';
      return;
    }

    // Now that the header is in the DOM, initialize all interactive components
    initializeComponents();
    // Dispatch event to notify that header is fully ready
