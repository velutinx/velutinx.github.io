...
(function() {
    ...
    // ----- Fit canvas inside wrapper (contain, with upscaling to fill) -----
    function fitCanvas(img) {
        ...
        // Compute scale to fit inside (contain) – allows upscaling if image is smaller
        const scaleX = availWidth / imgW;
        const scaleY = availHeight / imgH;
        canvasScale = Math.min(scaleX, scaleY); // no artificial limit
