...
(function() {
    ...
    async function handleAddClick() {
        ...
        if (success) {
            ...
            // If a new recurring expense was added, immediately fill any missing past occurrences
    ...
    async function ensurePastRecurringOccurrences() {
        ...
        for (const e of entries) {
            ...
            let month = e.month;
            let year = currentYear; // assume same year for now
            while (true) {
                // Stop if we hit today or later
                ...
                if (!exists) {
                    await addEntry({
                        ...
                        category: 'Expenses',
                        concept: e.concept,
                        recurring: false   // they are one‑time copies
                    });
                }
    ...
    function isAutoGeneratedRecurrence(entry) {
        // A past entry is considered a generated recurrence if it matches a recurring expense
        // with the same concept/amount, the same day of month, and its month > the original's month.
        const recExpenses = entries.filter(e => e.category === 'Expenses' && e.recurring);
        return recExpenses.some(rec => {
            if (rec.concept !== entry.concept || rec.amount !== entry.amount || rec.day !== entry.day) return false;
            // Check that this entry's month is after the recurring entry's start month (or it's a later month in same year)
            return entry.month > rec.month;  // simple assumption, works within same year
        });
    }
            ...
            for (const row of monthRows) {
                ...
                if (entry.category === 'Expenses') {
                    if (row.isFuture) {
                        ...
                        // Check if this entry is an autogenerated past recurrence
                        const isAuto = isAutoGeneratedRecurrence(entry);
                        if (isAuto) {
                            // Show a green checkmark, but not clickable
                            recurringHtml = `<span style="color:#4caf50; font-weight:bold;">✔</span>`;
                        } else {
