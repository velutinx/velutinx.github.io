...
function ensureToastContainer() {// cloudflare-pack-add-v2.js - Clean ASCII version (no emojis)
const WORKER_URL = 'https://pack-list.velutinx.workers.dev/api/packs';
...
function showToast(msg, type) {
    ...
    setTimeout(() => {
        t.style.cssText = 'opacity:0;transform:translateX(30px)';
...
function escapeHtml(s) {
    return String(s).replace(/[&<>]/g, m => m === '&' ? '&amp;' : m === '<' ? '&lt;' : m === '>' ? '&gt;' : m);
}
...
function renderTable(packs) {
    ...
    tbody.innerHTML = packs.map(p => {
        ...
        return `<tr><td><code>${escapeHtml(p.id)}</code></td><td>${escapeHtml(p.title)}</td><td><span class="category-badge ${catClass}">${catText}</span></td><td>${escapeHtml(p.price)}</td><td>${p.illustrationCount}</td><td>${dl}</td></tr>`;
    }).join('');
}
...
async function storePack(entry) {
    ...
    formData.append('price', entry.price);
    formData.append('illustrationCount', entry.illustrationCount);
    formData.append('downloadUrl', entry.downloadUrl || '');
    const r = await fetch(WORKER_URL, { method: 'POST', body: formData });
...
async function processZip(file) {
    ...
    try {
        ...
        let count = 0;
        zip.forEach((_, entry) => {
            if (!entry.dir && /\.(jpg|jpeg|png|gif|webp)$/i.test(entry.name)) count++;
        });
        ...
        await storePack({ id, title, category, price, illustrationCount: count, downloadUrl: downloadUrl || null });

        status.textContent = `Pack #${packNum} stored | ${count} images`;
...
function init() {
    ...
    dropzone.addEventListener('drop', e => {
        ...
        if (file && file.name.toLowerCase().endsWith('.zip')) processZip(file);
    ...
    if (toggle) toggle.addEventListener('change', () => {
        if (fileInput.files && fileInput.files.length) processZip(fileInput.files[0]);
    });
