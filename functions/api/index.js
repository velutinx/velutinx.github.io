export async function onRequest(context) {
  ...
  const PRODUCTS = {
    ...
    // add more packs here later (pack004 etc.)
  };
  // ===============================
  ...
  if (action === "create" && request.method === "POST") {
    ...
    for (const item of cart) {
      if (!PRODUCTS[item.id]) return new Response("Invalid product", { status: 400 });
  ...
  // ===============================
  // CAPTURE ORDER — NOW SAVES FULL CART
  // ===============================
  if (action === "capture" && request.method === "POST") {
    const { orderID, cart } = await request.json();
    if (!orderID || !cart) return new Response("Invalid request", { status: 400 });
    ...
    await fetch(`${env.SUPABASE_URL}/rest/v1/purchases`, {
      ...
      body: JSON.stringify({
        ...
        order_id: orderID,
        amount: serverTotal,
        cart: cart,                    // ← FULL CART SAVED HERE
  ...
  // ===============================
  // NEW: GET DOWNLOADS (multiple buttons)
  // ===============================
  if (action === "get-downloads" && request.method === "GET") {
    ...
    for (const item of order.cart || []) {
      const packKey = item.id;                    // "pack001"
      const link = env[packKey.toUpperCase()];    // env.PACK001
      if (link) {
        links.push({ name: `Download ${packKey}`, url: link });
      }
    }
  ...
  // ===============================
  // OLD DOWNLOAD (still kept for backwards)
  // ===============================
  if (action === "download") {
